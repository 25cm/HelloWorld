...
namespace FukjTabletSystem.Application.Boundary.Common
{
    ...
    public partial class GetPhotoDialog : Form
    {
...

        {
            ...
            {
                // TODO: とりあえず
                TabMessageBox.Show2("カメラ機能はWindows8以降で対応予定です");
            ...
            // TODO: スクリーンキーボードを非表示で起動しておきたい
            // カメラ起動の為にvbsからSendKeyしているがキーボードが起動していないとSendKeyが発生しない？
        ...
        private void stopButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (FileInfo file in targetDirectoryInfo.GetFiles())
                {
                    ...
                    {
                        TabMessageBox.Show2(Path.Combine(SaveDirectry, file.Name));

                        // ファイルを移動
                        file.MoveTo(Path.Combine(SaveDirectry, file.Name));
                        // リストに追加
                        ImageFileList.Add(Path.Combine(SaveDirectry, file.Name));
                    }
                }
            }
            catch
