...
{
    public partial class DaiBunruiSentakuForm : TabBaseForm
    {
        #region フォームデータ定義

        public class DaiBunruiSentakuFormData : FormData
        {
            public static string TANI_SOCHI_CD = "TaniSochiCd";
            public static string TANI_SOCHI_NM = "TaniSochiNm";

            public static string TANI_SOCHI_GRP_CD = "TaniSochiGroupCd";
            public static string TANI_SOCHI_GRP_NM = "TaniSochiGroupNm";
        ...
        public DaiBunruiSentakuForm()
        {
            InitializeComponent();
        }

        private void DaiBunruiSentakuForm_Load(object sender, EventArgs e)
        {
            // 画面遷移時の登録処理も自動生成する
            ...
            elementLayoutPanel.Controls.Remove(elementPanel2);
            elementLayoutPanel.Controls.Remove(elementPanel3);
            elementLayoutPanel.Controls.Remove(elementPanel4);
            elementLayoutPanel.Controls.Remove(elementPanel5);
        ...
        {
            FormData ret = new DaiBunruiSentakuFormData();
        ...
        public void SetButtonValueCallBack(Button button, string formDatakey, string value, FormData formData)
        {
            button.Click +=
                delegate(object sender, EventArgs e)
                {
                    ...
                    // TODO 汎用処理にする
                    formData.SetValue(DaiBunruiSentakuFormData.TANI_SOCHI_NM, ((Button)sender).Text);
        ...
        public void SetButtonEntryCallBack(Button button, string sochiCd, string sochiNm, TextBox value1, TextBox value2, FormData formData)
        {
            button.Click +=
                delegate(object sender, EventArgs e)
                {
                    ...
                    // TODO 汎用処理にできるとベター
                    //SetFormData(formData);
                    (formData as DaiBunruiSentakuFormData).SetTaniSochiScumValue(sochiCd, value1.Text, value2.Text);
                    ...
                    // TODO メッセージは仮
                    //MessageBox.Show(sochiNm + "の登録を行いました。");
        ...
        private Panel CreateElement(string taniSoutiName, string taniSoutiCd, string taniSoutiGrpCd, string taniSoutiGrpNm, FormData formData)
        {
            #region デザインパラメータ設定
            ...
            // 個別パラメータ設定
            taniSoutiButton.Text = taniSoutiGrpNm;
            ...
            string scumFrom;
            string scumTo;
            (formData as DaiBunruiSentakuFormData).GetTaniSochiScumValue(taniSoutiCd, out scumFrom, out scumTo);

            scumFromTextBox.Text = scumFrom;
            scumToTextBox.Text = scumTo;

            // 単位装置ボタンイベント設定
            SetButtonValueCallBack(taniSoutiButton, DaiBunruiSentakuFormData.TANI_SOCHI_GRP_CD, taniSoutiGrpCd, formData);
        ...
        private Panel CreateElement(DataRow row, FormData formData)
        {
            ...
            return CreateElement(tanisoutiName, tanisoutiCd, tanisoutiGrpCd, tanisoutiGrpName, formData);
