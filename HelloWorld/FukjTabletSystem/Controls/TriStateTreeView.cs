	...
	public class TriStateTreeView : TreeView
	{
		...
		private void PaintTreeNode( TriStateTreeNode node, Graphics gx )
		{
			if( this.CheckBoxes )
			{
				...
				Rectangle ncRect = new Rectangle( node.Bounds.X - 64, node.Bounds.Y + 16, 31, 31 );
		...
		private void DrawNodeLines( TriStateTreeNode node, Rectangle bounds, Graphics gx )
		{
			...
			using( Pen pen = new Pen( SystemColors.ControlDark, 1 ))
			{
				...
				gx.DrawLine( pen, new Point( bounds.X, bounds.Y + 16), new Point( bounds.X + 31, bounds.Y + 16 ));
				if( lineType == NodeLineType.WithChildren && node.IsExpanded )
				{
					gx.DrawLine( pen, new Point( bounds.X + 16, bounds.Y + 16 ), new Point( bounds.X + 16, bounds.Y + 32 ));
				}
			}
		}
        ...
        void MyTreeView_NodeMouseClick(object sender, TreeNodeMouseClickEventArgs e)
        {
            ...
            //if (e.Node != null && e.Node.Nodes.Count > 0)
            {
                    ...
                    HitTest(e.Location).Location == TreeViewHitTestLocations.Image)
                {
                    ...
                    //{
                    //    foreach (TriStateTreeNode node in this.Nodes)
                    //    {
                    //        closeNode(node);
                    //    }
                    //}
        ...
        void closeNode(TriStateTreeNode node)
        {
            if (node.IsExpanded)
            {
                node.Toggle();
