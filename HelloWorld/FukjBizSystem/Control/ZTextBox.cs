...
namespace FukjBizSystem.Control
{
    ...
    public partial class ZTextBox : TextBox
    {
        ...
        private void SetControlProperty()
        {
            ...
            //}
            //else 
            // TODO IMEControlを設定する(必要な場合)
            if (controlDomain == ZControlDomain.ZT_STD_NUM)
            {
                ImeMode = ImeMode.Off;
                KeyPress += InputValidateUtility.CreateKeyPressFilter("9");
            }
            else if (controlDomain == ZControlDomain.ZT_STD_CD)
            {
                ImeMode = ImeMode.Off;
                KeyPress += InputValidateUtility.CreateKeyPressFilter("A9");
            }
            else if (controlDomain == ZControlDomain.ZT_STD_NAME)
            {
