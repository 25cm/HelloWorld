...
namespace FukjBizSystem.Application.Boundary.KinoHoshoKanri
{
    ...
    /// </history>
    ////////////////////////////////////////////////////////////////////////////
    public partial class HoshoShinseiListForm : Form
    {
        ...
        private void shosaiButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string kenshakikan = kinoHoshoKanriListDataGridView.CurrentRow.Cells["HoshoTorokuKensakikanNoCol"].Value.ToString();
                string nendo = kinoHoshoKanriListDataGridView.CurrentRow.Cells["HoshoTorokuNendoNoCol"].Value.ToString();
                string renban = kinoHoshoKanriListDataGridView.CurrentRow.Cells["HoshoTorokuRenbanNoCol"].Value.ToString();
        ...
        #region inputCheckButton_Click
        ////////////////////////////////////////////////////////////////////////////
        //  イベント名 ： inputCheckButton_Click
        /// <summary>
        /// 
        /// </summary>
        /// <param name="e"></param>
        ...
        /// </history>
        ////////////////////////////////////////////////////////////////////////////
        private void inputCheckButton_Click(object sender, EventArgs e)
        {
            TraceLog.StartWrite(MethodInfo.GetCurrentMethod());
            Cursor preCursor = Cursor.Current;
        ...
        private void kinoHoshoKanriListDataGridView_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            try
            {
                ...
                string kenshakikan = kinoHoshoKanriListDataGridView.CurrentRow.Cells["HoshoTorokuKensakikanNoCol"].Value.ToString().Trim();
                string nendo = kinoHoshoKanriListDataGridView.CurrentRow.Cells["HoshoTorokuNendoNoCol"].Value.ToString();
                string renban = kinoHoshoKanriListDataGridView.CurrentRow.Cells["HoshoTorokuRenbanNoCol"].Value.ToString().Trim();
        ...
        private void OutputExcel(DataGridView targetDataGridView, string outputFilename)
        {
            ...
            try
            {
                ...
                for (int i = 0; i <= targetDataGridView.Rows.Count - 1; i++)
                {
                    ...
                    //C列 = 保証登録年度(和暦2桁)
                    CellOuputFormat(renbanWorkSheet, rowIdx + 1, 3, cell["HoshoTorokuNendoNoCol"].Value.ToString());

                    //D列 = 保証登録連番
                    CellOuputFormat(renbanWorkSheet, rowIdx + 1, 4, cell["HoshoTorokuRenbanNoCol"].Value.ToString());
                    ...
                    csvWorkSheet.Cells[rowIdx, 1] = Common.Common.ConvertToHoshouNendoSeireki(cell["HoshoTorokuNendoNoCol"].Value.ToString());
                    //csvWorkSheet.Cells[rowIdx, 1] = Utility.Utility.ConvertToSeireki(Int32.Parse(cell["HoshoTorokuNendoNoCol"].Value.ToString()));
                    // 20140811 habu End

                    //B列 = 保証登録連番
                    CellOuputFormat(csvWorkSheet, rowIdx, 2, cell["HoshoTorokuRenbanNoCol"].Value.ToString());

                    //C列 = 登録確認年(西暦4桁)
                    CellOuputFormat(csvWorkSheet, rowIdx, 3, cell["TorokuKakuninDtCol"].Value.ToString().PadLeft(10, ' ').Substring(0, 4));

                    //D列 = 登録確認月
                    CellOuputFormat(csvWorkSheet, rowIdx, 4, cell["TorokuKakuninDtCol"].Value.ToString().PadLeft(10, ' ').Substring(5, 2));

                    //E列 = 登録確認日
                    CellOuputFormat(csvWorkSheet, rowIdx, 5, cell["TorokuKakuninDtCol"].Value.ToString().PadLeft(10, ' ').Substring(8, 2));

                    //F列 = 使用年(西暦4桁)
                    //CellOuputFormat(csvWorkSheet, rowIdx, 6, cell["ShiyoKaisiDtCol"].Value.ToString().PadLeft(10, ' ').Substring(0, 4));
                    csvWorkSheet.Cells[rowIdx, 6] = cell["ShiyoKaisiDtCol"].Value.ToString().PadLeft(10, ' ').Substring(0, 4);

                    //G列 = 使用月
                    CellOuputFormat(csvWorkSheet, rowIdx, 7, cell["ShiyoKaisiDtCol"].Value.ToString().PadLeft(10, ' ').Substring(5, 2));

                    //H列 = 使用日
                    CellOuputFormat(csvWorkSheet, rowIdx, 8, cell["ShiyoKaisiDtCol"].Value.ToString().PadLeft(10, ' ').Substring(8, 2));

                    //I列 = 全浄連登録番号
                    CellOuputFormat(csvWorkSheet, rowIdx, 9, cell["JokasoZenjyokyoTorokuNoCol"].Value.ToString());

                    //J列 = 人槽
                    CellOuputFormat(csvWorkSheet, rowIdx, 10, cell["JokasoNinsoCol"].Value.ToString());

                    //K列 = 補助市町村
                    CellOuputFormat(csvWorkSheet,rowIdx, 11, cell["ShichosonCdCol"].Value.ToString());
        ...
        private void ConvertNendoToHeisei()
        {
            ...
            {
                // 20140811 habu Aggregated Hoshou Toroku No Convertion To Common Utility
                row.Cells["HoshoTorokuNendoNoCol"].Value = Common.Common.ConvertToHoshouNendoWareki(row.Cells["HoshoTorokuNendoNoCol"].Value.ToString());
