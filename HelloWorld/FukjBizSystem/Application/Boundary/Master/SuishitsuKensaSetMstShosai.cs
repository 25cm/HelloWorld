...
namespace FukjBizSystem.Application.Boundary.Master
{
    ...
    /// </history>
    ////////////////////////////////////////////////////////////////////////////
    public partial class SuishitsuKensaSetMstShosaiForm : Form
    {
        #region 定義(public)
            ...
            try
            {
                ...
                if (e.ColumnIndex == 8)
                {
                    if (!Utility.Utility.IsDecimal(setShikenListDataGridView.CurrentRow.Cells[e.ColumnIndex].Value.ToString()))
                    {
                        ...
                        setShikenListDataGridView.CurrentRow.Cells["SetKomokuRyokinCol"].Value = 0;
                    }
                    else
                    {
        ...
        private bool DataCheck()
        {
            ...
            {
                errMsg.Add("必須項目：セット名称（略式）が入力されていません。\r\n");
            }
            else if (setNmRyakushoTextBox.Text.Length > 30)
            {
                errMsg.Add("セット名称（略式）は30文字以下で入力して下さい。\r\n");
            }
            ...
            foreach (DataGridViewRow dgvr in setShikenListDataGridView.Rows)
            {
                ...
                if (dgvr.Cells["SetKomokuCdCol"].Value == null 
                    || string.IsNullOrEmpty(dgvr.Cells["SetKomokuCdCol"].Value.ToString().Trim()))
                {
                    requireLines.Add(rowCnt + 1 + string.Empty);
                }
                else if (!Utility.Utility.IsDecimal(dgvr.Cells["SetKomokuCdCol"].Value.ToString()))
                {
                    notNumLines.Add(rowCnt + 1 + string.Empty);
                }
                else if (dgvr.Cells["SetKomokuCdCol"].Value.ToString().Length != 3)
                {
        ...
        private bool EditCheck()
        {
            ...
            foreach (DataGridViewRow dgvr in setShikenListDataGridView.Rows)
            {
                ...
                // セット試験項目コード(8)
                if (!dgvr.Cells["SetKomokuCdCol"].Value.Equals(dgvr.Cells["DefaultSetKomokuCdCol"].Value))
                {
                    return false;
                }

                // 証明書記載区分(11)
                if (!dgvr.Cells["ShomeishoKisaiKbnCol"].Value.Equals(dgvr.Cells["DefaultShomeishoKisaiKbnCol"].Value))
                {
                    return false;
                }

                // 請求有無区分(12)
                if (!dgvr.Cells["SetKomokuSeikyuUmuKbnCol"].Value.Equals(dgvr.Cells["DefaultSetKomokuSeikyuUmuKbnCol"].Value))
                {
                    return false;
                }
            }
        ...
        private bool EditControl()
        {
            ...
            if (!string.IsNullOrEmpty(setCdTextBox.Text)
                ...
                || (setShikenListDataGridView.Rows[0].Cells["SetKomokuCdCol"].Value != null
                    && !string.IsNullOrEmpty(setShikenListDataGridView.Rows[0].Cells["SetKomokuCdCol"].Value.ToString()))
                || (setShikenListDataGridView.Rows[0].Cells["ShomeishoKisaiKbnCol"].Value != null 
                    && setShikenListDataGridView.Rows[0].Cells["ShomeishoKisaiKbnCol"].Value.ToString() == "1")
                || (setShikenListDataGridView.Rows[0].Cells["SetKomokuSeikyuUmuKbnCol"].Value != null
                    && setShikenListDataGridView.Rows[0].Cells["SetKomokuSeikyuUmuKbnCol"].Value.ToString() == "1"))
            {
                return true;
            }
        ...
        private void SetRyokin()
        {
            ...
            {
                // Skip unchecked rows
                if ((bool)dgvr.Cells["SetKomokuSeikyuUmuKbnCol"].FormattedValue == false) continue;

                // Sum all checked rows
                setRyokin += (dgvr.Cells["SetKomokuRyokinCol"].Value != null && !string.IsNullOrEmpty(dgvr.Cells["SetKomokuRyokinCol"].Value.ToString())) ?
                    Convert.ToDecimal(dgvr.Cells["SetKomokuRyokinCol"].Value) : 0;
            }
        ...
        private void DgvControl()
        {
            ...
            {
                if (dgvr.Index == setShikenListDataGridView.Rows.Count - 1) break;
                dgvr.Cells["DefaultSetKomokuCdCol"].Value = _setShikenDT[dgvr.Index].SetKomokuCd;
                dgvr.Cells["DefaultShomeishoKisaiKbnCol"].Value = _setShikenDT[dgvr.Index].ShomeishoKisaiKbn;
                dgvr.Cells["DefaultSetKomokuSeikyuUmuKbnCol"].Value = _setShikenDT[dgvr.Index].SetKomokuSeikyuUmuKbn;
            }
        }
        #endregion
            ...
            for (int currentRow = 0; currentRow < setShikenListDataGridView.Rows.Count - 1; currentRow++)
            {
                ...
                for (int otherRow = 0; otherRow < setShikenListDataGridView.Rows.Count - 1; otherRow++)
                {
                    ...
                    if (currentRow != otherRow
                        && !string.IsNullOrEmpty(rowToCompare.Cells["SetKomokuCdCol"].Value.ToString())
                        && !string.IsNullOrEmpty(row.Cells["SetKomokuCdCol"].Value.ToString())
                        && rowToCompare.Cells["SetKomokuCdCol"].Value.Equals(row.Cells["SetKomokuCdCol"].Value))
                    {
                        duplicateRow = true;
                    }
        ...
        private void SetKomokuNmColTextChanged()
        {
            ...
            if (setShikenListDataGridView.CurrentRow.Cells["SetKomokuCdCol"].Value == null
                || string.IsNullOrEmpty(setShikenListDataGridView.CurrentRow.Cells["SetKomokuCdCol"].Value.ToString()))
            {
                // セット試験項目名称(9)
                setShikenListDataGridView.CurrentRow.Cells["SetKomokuNmCol"].Value = string.Empty;
            ...
            if (0 == alOutput.SuishitsuShikenKoumokuMstDataTable.Count)
            {
                ...
                // 水質試験項目料金(10)
                setShikenListDataGridView.CurrentRow.Cells["SetKomokuRyokinCol"].Value = false;

                return;
            }

            // セット試験項目名称(9)
            DataGridViewComboBoxCell cb = (DataGridViewComboBoxCell)setShikenListDataGridView.CurrentRow.Cells["SetKomokuNmCol"];
            cb.Value = setShikenListDataGridView.CurrentCell.Value.ToString();

            // 水質試験項目料金(10)
            setShikenListDataGridView.CurrentRow.Cells["SetKomokuRyokinCol"].Value = alOutput.SuishitsuShikenKoumokuMstDataTable[0].SuishitsuShikenKomokuAmt;
        }
        #endregion
        ...
        private void ComboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            // セット試験項目コード(8)
            setShikenListDataGridView.CurrentRow.Cells["SetKomokuCdCol"].Value = cb.SelectedValue;
        ...
        private SuishitsuKensaSetMstDataSet.SuishitsuKensaSetMstDataTable CreateSuishitsuKensaSetMstInsert()
        {
            ...
            newRow.SetCd = Common.Common.GetKeyRenban("SuishitsuKensaSetMst", "", "", "").PadLeft(3, '0');
            // UPD 20140724 END ZynasSou
        ...
        private SetShikenKomokuMstDataSet.SetShikenKomokuMstDataTable CreateSetShikenKomokuMstInsert(SuishitsuKensaSetMstDataSet.SuishitsuKensaSetMstDataTable setMstDT)
        {
            ...
            foreach (DataGridViewRow dgvr in setShikenListDataGridView.Rows)
            {
                ...
                // セット試験項目コード(8)
                newRow.SetKomokuCd = dgvr.Cells["SetKomokuCdCol"].Value.ToString();

                // 証明書記載区分(11)
                newRow.ShomeishoKisaiKbn = !string.IsNullOrEmpty(dgvr.Cells["ShomeishoKisaiKbnCol"].Value.ToString()) ?
                    dgvr.Cells["ShomeishoKisaiKbnCol"].Value.ToString() : "2";

                // 請求有無区分(12)
                newRow.SetKomokuSeikyuUmuKbn = !string.IsNullOrEmpty(dgvr.Cells["SetKomokuSeikyuUmuKbnCol"].Value.ToString()) ?
                    dgvr.Cells["SetKomokuSeikyuUmuKbnCol"].Value.ToString() : "2";
        ...
        private SetShikenKomokuMstDataSet.SetShikenKomokuMstDataTable CreateSetShikenKomokuMstUpdate(SetShikenKomokuMstDataSet.SetShikenKomokuMstDataTable table)
        {
            ...
            foreach (DataGridViewRow dgvr in setShikenListDataGridView.Rows)
            {
                ...
                {
                    // セット試験項目コード(8)
                    table[dgvr.Index].SetKomokuCd = dgvr.Cells["SetKomokuCdCol"].Value.ToString();

                    // 証明書記載区分(11)
                    table[dgvr.Index].ShomeishoKisaiKbn = !string.IsNullOrEmpty(dgvr.Cells["ShomeishoKisaiKbnCol"].Value.ToString()) ?
                        dgvr.Cells["ShomeishoKisaiKbnCol"].Value.ToString() : "2";

                    // 請求有無区分(12)
                    table[dgvr.Index].SetKomokuSeikyuUmuKbn = !string.IsNullOrEmpty(dgvr.Cells["SetKomokuSeikyuUmuKbnCol"].Value.ToString()) ?
                        dgvr.Cells["SetKomokuSeikyuUmuKbnCol"].Value.ToString() : "2";
