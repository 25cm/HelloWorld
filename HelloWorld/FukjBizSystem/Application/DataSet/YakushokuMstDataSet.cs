...
namespace FukjBizSystem.Application.DataSet.YakushokuMstDataSetTableAdapters
{
    ...
    public partial class YakushokuMstKensakuTableAdapter 
    {
            ...
            String yakushokuNm)
        {
            ...
            {
                sqlContent.Append("AND NameMst.NameCd = @nameCd                         ");
                commandParams.Add("@nameCd", SqlDbType.Char).Value = nameCd;
            }

            sqlContent.Append("AND YakushokuMst.YakushokuCd " + DataAccessUtility.SetBetweenCommand(yakushokuCdFrom, yakushokuCdTo, 2));
            //if (!String.IsNullOrEmpty(yakushokuCdFrom) && !String.IsNullOrEmpty(yakushokuCdTo))
            //{
            //    sqlContent.Append("AND YakushokuMst.YakushokuCd >= CAST(@yakushokuCdFrom AS INT) AND YakushokuMst.YakushokuCd <= CAST(@yakushokuCdTo AS INT) ");
            //    commandParams.Add("@yakushokuCdFrom", SqlDbType.Char).Value = yakushokuCdFrom;
            //    commandParams.Add("@yakushokuCdTo", SqlDbType.Char).Value = yakushokuCdTo;
            //}
