...
{
    public partial class ShoruiKensaEntryForm : TabBaseForm
    {
        #region フォームデータ定義

        public class ShoruiKensaEntryFormData : FormData
        {
        ...
        public ShoruiKensaEntryForm()
        {
            InitializeComponent();
        }

        private void ShoruiKensaEntryForm_Load(object sender, EventArgs e)
        {
            // 画面遷移時の登録処理も自動生成する
        ...
        {
            FormData ret = new ShoruiKensaEntryFormData();
        ...
        public void SetButtonValueCallBack(Button button, string formDatakey, string value, FormData formData)
        {
            button.Click +=
                delegate(object sender, EventArgs e)
                {
                    ...
                    // TODO 汎用処理にする
                    formData.SetValue(ShoruiKensaEntryFormData.TANI_SOCHI_NM, ((Button)sender).Text);
        ...
        public void SetButtonEntryCallBack(Button button, string sochiCd, string sochiNm, TextBox value1, TextBox value2, FormData formData)
        {
            button.Click +=
                delegate(object sender, EventArgs e)
                {
                    ...
                    // TODO 汎用処理にできるとベター
                    //SetFormData(formData);
                    (formData as ShoruiKensaEntryFormData).SetTaniSochiScumValue(sochiCd, value1.Text, value2.Text);
        ...
        private Panel CreateElement(string taniSoutiName, string taniSoutiCd, string taniSoutiGrpCd, FormData formData)
        {
            ...
            string scumFrom;
            string scumTo;
            (formData as ShoruiKensaEntryFormData).GetTaniSochiScumValue(taniSoutiCd, out scumFrom, out scumTo);

            scumFromTextBox.Text = scumFrom;
            scumToTextBox.Text = scumTo;

            // 単位装置ボタンイベント設定
            SetButtonValueCallBack(taniSoutiButton, ShoruiKensaEntryFormData.TANI_SOCHI_CD, taniSoutiGrpCd, formData);
        ...
        {
            if (((string)hoshuGyoshaComboBox.SelectedItem) == "0001")
            {
                hoshuGyoshaNmTextBox.Text = "保守業者A";
            }
            if (((string)hoshuGyoshaComboBox.SelectedItem) == "0002")
            {
                hoshuGyoshaNmTextBox.Text = "保守業者B";
            }
            if (((string)hoshuGyoshaComboBox.SelectedItem) == "0003")
            {
                hoshuGyoshaNmTextBox.Text = "保守業者C";
            }
            if (((string)hoshuGyoshaComboBox.SelectedItem) == "0004")
            {
                hoshuGyoshaNmTextBox.Text = "保守業者D";
            }
            if (((string)hoshuGyoshaComboBox.SelectedItem) == "0005")
            {
                hoshuGyoshaNmTextBox.Text = "保守業者E";
            }
            if (((string)hoshuGyoshaComboBox.SelectedItem) == "0006")
            {
                hoshuGyoshaNmTextBox.Text = "保守業者F";
            }
        }

        private void comboBox2_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (((string)seisouGyoshaComboBox.SelectedItem) == "0001")
            {
                seisouGyoshaNmTextBox.Text = "清掃業者A";
            }
            if (((string)seisouGyoshaComboBox.SelectedItem) == "0002")
            {
                seisouGyoshaNmTextBox.Text = "清掃業者B";
            }
            if (((string)seisouGyoshaComboBox.SelectedItem) == "0003")
            {
                seisouGyoshaNmTextBox.Text = "清掃業者C";
            }
            if (((string)seisouGyoshaComboBox.SelectedItem) == "0004")
            {
                seisouGyoshaNmTextBox.Text = "清掃業者D";
            }
            if (((string)seisouGyoshaComboBox.SelectedItem) == "0005")
            {
                seisouGyoshaNmTextBox.Text = "清掃業者E";
            }
            if (((string)seisouGyoshaComboBox.SelectedItem) == "0006")
            {
                seisouGyoshaNmTextBox.Text = "清掃業者F";
            }
        }
