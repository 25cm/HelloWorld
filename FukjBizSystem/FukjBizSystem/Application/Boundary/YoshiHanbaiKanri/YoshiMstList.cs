...
namespace FukjBizSystem.Application.Boundary.YoshiHanbaiKanri
{
    ...
    /// </history>
    ////////////////////////////////////////////////////////////////////////////
    public partial class YoshiMstListForm : Form
    {
        ...
        private bool InputCheck()
        {
            ...
            for (int i = 0; i < shiireListDataGridView.RowCount - 1; i++)
            {
                ...
                else
                {
                    ...
                    if ((row.Cells["KaiinSetKakakuCol"].Value == null || string.IsNullOrEmpty(row.Cells["KaiinSetKakakuCol"].Value.ToString()))
                        && (row.Cells["HiKaiinSetKakakuCol"].Value == null || string.IsNullOrEmpty(row.Cells["HiKaiinSetKakakuCol"].Value.ToString()))
                        && (row.Cells["SetBusuCol"].Value == null || string.IsNullOrEmpty(row.Cells["SetBusuCol"].Value.ToString())))
                    {
                        // don't handled
                    }
                    else if ((row.Cells["KaiinSetKakakuCol"].Value == null || string.IsNullOrEmpty(row.Cells["KaiinSetKakakuCol"].Value.ToString()))
                        || (row.Cells["HiKaiinSetKakakuCol"].Value == null || string.IsNullOrEmpty(row.Cells["HiKaiinSetKakakuCol"].Value.ToString()))
                        || (row.Cells["SetBusuCol"].Value == null || string.IsNullOrEmpty(row.Cells["SetBusuCol"].Value.ToString())))
                    {
            ...
            {
                errMess.Append("行 : " + emptyKaiinSetKakaku.Remove(emptyKaiinSetKakaku.Length - 2) + " : セット価格、非会員セット価格、セット部数を入力してください。\r\n");
            }
