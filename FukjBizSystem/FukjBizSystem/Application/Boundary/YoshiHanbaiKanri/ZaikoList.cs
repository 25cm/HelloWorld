...
namespace FukjBizSystem.Application.Boundary.YoshiHanbaiKanri
{
    ...
    /// </history>
    ////////////////////////////////////////////////////////////////////////////
    public partial class ZaikoListForm : Form
    {
        ...
        private void zaikoListDataGridView_EditingControlShowing(object sender, DataGridViewEditingControlShowingEventArgs e)
        {
            ...
            try
            {
                ...
                if (zaikoListDataGridView.Columns[zaikoListDataGridView.CurrentCell.ColumnIndex].Name == "SuryoCdCol")
                {
        ...
        private DataGridView CreateDataOutput()
        {
            ...
            for (int rowIdx = 0; rowIdx < zaikoListDataGridView.RowCount - 1; rowIdx++)
            {
                ...
                for (int colIdx = 0; colIdx < zaikoListDataGridView.ColumnCount; colIdx++)
                {
                    ...
                    if (zaikoListDataGridView.Columns[colIdx].Name == "DeleteFlgCol")
                    {
                        row.Cells[colIdx].Value = (zaikoListDataGridView.Rows[rowIdx].Cells["DeleteFlgCol"].Value == null) ? "0" : "1";
                        continue;
                    }
                ...
                dgvOutput.Columns["DeleteFlgCol"].DisplayIndex = 3;
        ...
        private bool IsRegisConfirm()
        {
            for (int i = 0; i < zaikoListDataGridView.RowCount - 1; i++)
            {
                ...
                if (row.Cells["SuryoCdCol"].Value == null || !Utility.Utility.IsDecimal(row.Cells["SuryoCdCol"].Value.ToString())) continue;

                if (Int32.Parse(row.Cells["SuryoCdCol"].Value.ToString()) > 0 && (row.Cells["DeleteFlgCol"].Value != null && row.Cells["DeleteFlgCol"].Value.ToString() == "1"))
                {
        ...
        private bool IsValidDataUpdate()
        {
            ...
            else
            {
                for (int i = 0; i < zaikoListDataGridView.RowCount - 1; i++)
                {
                    ...
                    //支所(新規追加行のみ)(10)
                    if (row.Cells["ShishoCol"].Value == null || string.IsNullOrEmpty(row.Cells["ShishoCol"].Value.ToString()))
                    {
                        emptyShishoCd += (i + 1) + ", ";
                    }

                    //用紙(新規追加行のみ)(11)
                    if (row.Cells["YoshiCol"].Value == null || string.IsNullOrEmpty(row.Cells["YoshiCol"].Value.ToString()))
                    {
                        emptyYoshiCd += (i + 1) + ", ";
                    }

                    //在庫数(12)
                    if (row.Cells["SuryoCdCol"].Value == null || string.IsNullOrEmpty(row.Cells["SuryoCdCol"].Value.ToString()))
                    {
                        emptySuryo += (i + 1) + ", ";
                    }
                    else if (!Utility.Utility.IsDecimal(row.Cells["SuryoCdCol"].Value.ToString()))
                    {
                        incorrectSuryo += (i + 1) + ", ";
                    }
                }
        ...
        private List<string> GetDataDuplicated()
        {
            ...
            for (int i = startRow; i < zaikoListDataGridView.RowCount - 1; i++)
            {
                ...
                for (int k = i + 1; k < zaikoListDataGridView.RowCount - 1; k++)
                {
                    ...
                    if (shishoCd == rowK.Cells["ShishoCol"].Value.ToString()
                        && yoshiCd == rowK.Cells["YoshiCol"].Value.ToString()
        ...
        private void ChangeBackGroundCell(List<string> listKey)
        {
            ...
            foreach (string key in listKey)
            {
                ...
                {
                    DataGridViewRow row = zaikoListDataGridView.Rows[i];
                    if (row.Cells["ShishoCol"].Value.ToString() == shishoCd
                        && row.Cells["YoshiCol"].Value.ToString() == yoshiCd)
                    {
        ...
        private void SetReadonlyGridView()
        {
            for (int i = 0; i < zaikoListDataGridView.RowCount - 1; i++)
            {
                foreach (DataGridViewColumn col in zaikoListDataGridView.Columns)
                {
                    ...
                    if (col.Name == "ShishoCol" || col.Name == "YoshiCol")
                    {
                        cell.ReadOnly = true;
                    }

                    if (col.Name == "SuryoOldCol")
                    {
                        cell.Value = zaikoListDataGridView.Rows[i].Cells["SuryoCdCol"].Value;
                    }
                }
            }
        }
        ...
        private bool IsEditedControl()
        {
            for (int i = 0; i < zaikoListDataGridView.RowCount - 1; i++)
            {
                ...
                if ((row.Cells["DeleteFlgCol"].Value != null && row.Cells["DeleteFlgCol"].Value.ToString() == "1")
                    || row.Cells["SuryoOldCol"].Value != null && (row.Cells["SuryoCdCol"].Value.ToString() != row.Cells["SuryoOldCol"].Value.ToString())
